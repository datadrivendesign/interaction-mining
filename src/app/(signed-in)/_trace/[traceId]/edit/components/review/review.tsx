...
export default function Review() {
  ...
  return (
    <div className="flex h-full w-full">
      <div className="flex w-2/3 overflow-auto border-r border-neutral-200 dark:border-neutral-800">
        <SaveTraceGallery />
      </div>
      <div className="flex h-full w-1/3 shrink-0 grow-0 flex-col items-center justify-center p-8">
...
function SaveTraceGallery() {
  ...
  return (
    <div className="grid w-full grid-cols-2 items-start gap-4 p-8 md:grid-cols-3 lg:grid-cols-4">
      {screens.map((screen: Screen) => (
        <div
          className="relative flex flex-col rounded-xl bg-neutral-100 dark:bg-neutral-900"
          key={`${screen.id}`}
        >
          <Image
            className="z-0 h-auto w-full rounded-lg object-cover"
            src={screen.src}
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div
                  className="absolute flex aspect-square w-[12%] -translate-x-1/2 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full bg-green-300 opacity-70"
                  style={{
