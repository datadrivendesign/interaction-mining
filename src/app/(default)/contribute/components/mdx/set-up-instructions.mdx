...
The script checks if MongoDB Community Edition is installed by checking for a Mongo Shell (`mongosh`) installation. If you do not have `mongosh` installed on your machine, it will use install MongoDB Community Edition for you.
...
Next, the script will ask whether you want to set up the ODIM Android mobile app. If not, the script will skip the set up in this section

#### Java Set Up

Our Android app requires **Java JDK 17 or higher** to build. The script will install `OpenJDK v17` and add it to your `$PATH` variable if you do not have it or have the wrong version. 
...
The script will try to bundle an **Android APK** using a Gradle script in the mobile repo. The script requires parameter`API_URL_PREFIX`: the root url domain where the web app is hosted. 
